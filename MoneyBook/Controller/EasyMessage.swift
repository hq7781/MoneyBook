...
extension UIViewController {
    public func showMessage(_ text: String, type: MessageType, options: [MessageOption]? = nil) {
        EasyMessage.showMessageAddedTo(text, type: type, options: options, inView: view, inViewController: self)
    }
...
extension UIView {
    public func showMessage(_ text: String, type: MessageType, options: [MessageOption]? = nil) {
        EasyMessage.showMessageAddedTo(text, type: type, options: options, inView: self, inViewController: nil)
    }
...
//public protocol EasyMessageDelegate : class {
//    func popupMessageDidDismiss(_ popupMessage : PopupMessage)
//}
...
public class EasyMessage {
    ...
    public class func showMessageAddedTo(_ text: String, type: MessageType, options: [MessageOption]?, inView: UIView?, inViewController: UIViewController?) {
        
        if inView?.installedMessage != nil && inView?.uninstallMessage == nil { inView?.hideMessage() }
        if inView?.installedMessage == nil {
            EasyMessage(text:text, type: type, options: options!, inView: inView!, inViewController: inViewController).show()
        }
    }
    ...
    public fileprivate(set) var animation: MessageAnimation = .slide
    ...
    public fileprivate(set) var position: MessagePosition = .top
    ...
    public init(text: String, type: MessageType, options: [MessageOption]?,
                inView: UIView, inViewController: UIViewController?) {
    
        let inView = inView
        
        switch type {
            
        case .success: backgroundColor = EasyMessage.successBackgroundColor
        case .warning: backgroundColor = EasyMessage.warningBackgroundColor
        case .error: backgroundColor = EasyMessage.errorBackgroundColor
        case .info: backgroundColor = EasyMessage.infoBackgroundColor
        case .hint: backgroundColor = EasyMessage.infoBackgroundColor
        }
        ...
        messageText.font = EasyMessage.font
        ...
        if textNumberOfLines == 0 {
            ...
            height = max(text.boundingRect(with: CGSize(width: inView.frame.size.width - textPadding * 2,
                                                        height: .greatestFiniteMagnitude),
                                           options: .usesLineFragmentOrigin,
                                           attributes: [NSFontAttributeName: EasyMessage.font],
                                           context: nil).height + (height - " ".boundingRect(with: CGSize(width: inView.frame.size.width, height: .greatestFiniteMagnitude), options: .usesLineFragmentOrigin, attributes: [NSFontAttributeName: EasyMessage.font], context: nil).height),
                         height)
        }
        
        NotificationCenter.default.addObserver(self, selector: #selector(updateFrames),
    ...
    //  guard let inView = inView else { return }
        ...
        case .midle:
            y = (inView.bounds.size.height - height) / 2
        }
        
        messageView.frame = CGRect(x: 0, y: y, width: inView.bounds.size.width, height: messageHeight)
        messageText.frame = CGRect(x: textPadding, y: offsetY, width: messageView.bounds.size.width - textPadding * 2,
                                   height: height)
    }
...
extension UIView {
    fileprivate var installedMessage: EasyMessage? {
        get { return objc_getAssociatedObject(self, &installedMessageKey) as? EasyMessage }
        set { objc_setAssociatedObject(self, &installedMessageKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC) }
    }
    fileprivate var uninstallMessage: EasyMessage? {
        get { return objc_getAssociatedObject(self, &uninstallMessageKey) as? EasyMessage }
